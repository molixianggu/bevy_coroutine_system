...
- ⚠️ **重要安全限制**: 不要跨 `yield` 持有 `SystemParam` 的引用！这会导致未定义行为

### 🛡️ 安全使用指南

由于实现机制的限制，协程系统在内部使用了裸指针来绕过生命周期检查。这意味着你必须遵守以下规则：
...
#### 💡 最佳实践
1. **立即处理**: 在 `yield` 前完成所有需要引用的操作
2. **存储 ID**: 只存储 `Entity` 或其他 Copy 类型的值
3. **重新查询**: 在 `yield` 后重新获取需要的数据
